...
class HealthSpider(BaseSpider):
    ...
    def parse(self, response):
		...
		for health in healths:
			item = HealthItem()
			item['name'] = health.select('//td')[count*4].select('text()').extract()
			item['phone'] = health.select('//td')[count*4+1].select('ul/li')[0].select('text()').extract()
			item['phone'].append(health.select('//td')[count*4+1].select('ul/li')[1].select('text()').extract())
			item['address'] = health.select('//td')[count*4+2].select('text()').extract()
			item['home_page'] = health.select('//td')[count*4+3].select('a/@href').extract()
			count += 1
			items.append(item)
